...
const AllOrders = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const res = await axiosInstance.get(
        "http://localhost:3001/api/admin/get-all-orders",
        { headers }
      );
  ...
  const submitChanges = async (i) => {
    ...
    const response = await axiosInstance.put(
      `http://localhost:3001/api/admin/update-order/${id}`,
      Values,
      { headers }
    );
        ...
        <div className="h-[100%] p-0 md:p-4 text-zinc-100">
          ...
          {OrderHistory.map((items, i) => (
            <div className=" bg-zinc-800 w-full rounded py-2 px-4 flex gap-2 hover:bg-zinc-900 hover:cursor-pointer transition-all duration-300 ">
              ...
              <div className="w-0 md:w-[45%] hidden md:block">
                <h1 className="">{items.books.desc.slice(0, 50)} ...</h1>
              </div>
              <div className="w-[17%] md:w-[9%]">
                <h1 className="">₹ {items.books.price}</h1>
              </div>
              <div className="w-[30%] md:w-[16%]">
